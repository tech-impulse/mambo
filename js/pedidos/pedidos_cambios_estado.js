        ...
        tx.executeSql(sql, undefined,
            function (tx, result) {
                ...
                if (parseInt(localStorage['pLastInternalOrder']) > id) {
                    id = parseInt(localStorage['pLastInternalOrder']);
                    console.log("COGE EL LOCAL ");
                } else {
                    id = id + 1;
                    console.log("COGE EL BD");
                }
...
function pInsertarDetallePedidoTemporal(idItem, idLogisticChain, quantity, itemName, logisticsChainName) {
    ...
    console.log("INSETANDO ARTICULO ==> " + itemName + "|" + logisticsChainName + "|");
                    ...
                    tx.executeSql(sql, undefined,
                        function (tx, max) {
                            ...
                            if (max.rows.length > 0) {
                                ...
                                if (parseInt(localStorage['pLastInternalOrder']) > newId) {
                                    newId = parseInt(localStorage['pLastInternalOrder']) + 1;
                                } else {
                                    newId++;
                                }
                            } else {
                                if (parseInt(localStorage['pLastInternalOrder']) > 0) {
                                    newId = parseInt(localStorage['pLastInternalOrder']) + 1;
                                }
                            }
        ...
        tx.executeSql(sql, undefined,
            function (tx, result) {
                ...
                if (result.rows.length > 0) {
                    ...
                    if (zona == "" || zona == undefined) {} else {
                        z = 'idDeliveryZone="' + zona + '" , "';
                    }
