...
function pMostrarDetalleArticulo(datos, orden) {
    var codArticulo= datos.cod_pedid;;
    var nomArticulo= datos.nom_pedid;;
    ...
    db.transaction(function (transaction) {
        ...
        var pJsonAux = [];
        /*
        //ANTES DE TENERLOS TODO EN CATALOGO
        ...
        */
        ...
        if(localStorage["pantalla"]=="pedidosDetalleNuevoEscaner")
                	var sql = "SELECT l.logisticsChainName as desCantidad, 1 as numUds, l.idLogisticsChains, c.vendorReference, theoreticalStock, allowsPerUnitRequest, itemUnitName, c.* FROM catalog as c  WHERE c.idPurchaseCenter="+cen+" AND c.idItem="+codArticulo+"  ORDER BY logisticsChainName  DESC ";
                else
                	var sql = "SELECT logisticsChainName as desCantidad, 1 as numUds, c.* FROM catalog as c  WHERE c.idPurchaseCenter="+cen+" AND c.idVendor="+prov+" AND c.idItem="+codArticulo+" ORDER BY logisticsChainName  DESC ";
        ...
        transaction.executeSql(sql, undefined,
            function (transaction, result) {
                ...
                for (i = 0; i < result.rows.length; i++) {
                    ...
                    if (rowDb.desCantidad != rowDb.itemUnitName ) {
                ...
                if (rowDb.mainItemEan!= undefined ) { $("#InEANPrincipalInsertarPedido").val(rowDb.mainItemEan); }
                else { $("#InEANPrincipalInsertarPedido").val(""); }
        ...
        transaction.executeSql(sql, undefined,
            function (transaction, result) {
                if (result.rows.length > 0) {
                    ...
                    var longname = $('#LbTituloInsertarPedido').text();
                    var name = longname.substr(longname.indexOf("-") + 2);
                    var chainName = $('#InCadenaInsertarPedido').data("kendoDropDownList").text();
