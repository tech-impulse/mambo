...
function checkInicio() {
    ...
    if (localStorage["online"] == 1) // Estamos Online
    {
                ...
                function (transaction, result) {
                    ...
                    if (result.rows.length > 0) // Hay usuario guardado
                    {
                        ...
                        if ((usuarioAnterior == usuario) && (passAnterior == pass)) {
                            ...
                            if(compareTime() >= tiempoRecargaBD){//si han pasado las 12h preguntamos si desea actualizar
            ...
            function (transaction, result) {
                if (result.rows.length > 0) {
                    ...
                    if ((contDb == contService) && (scopes.length == result.rows.length)) {
                        if (compareTime() >= tiempoRecargaBD) //mismo scope 12h cumpludas debemos recargar todo de nuevo
                        {
                            if (localStorage['online'] == 1) {
                                console.log("Mismos Scopes, tiempo MAYOR a 12h estamos online");
                                var currentdate = new Date();
                                localStorage["ultima_carga"] = currentdate.getTime(); // guardamos otra vez la ultima vez para recalcular las 6h
                                localStorage.setItem('checkUsuario', "1");
                                restServices(res);
                            } else {
                                getDescripcionAviso("loginOfflinePregunta"); //no estamos online preguntamos si quiere actualizar la BB.DD
                                $("#loginDialogAC").popup("open");
                            }

                        } else //aun no se han cumplido las 12h
                        {
                            if (localStorage["cargaDeDatos"] == "completa") //y tenemos una carga de datos completada toda con exito
                            {
                                console.log("Mismos Scopes, tiempo menor a 6h");
                                localStorage['pantalla'] = "menuPrincipal";
                                $.mobile.changePage('#menuPrincipal');
                            } else //carga incompleta comprobamos el usuario
                            {
                                if (localStorage['online'] == 1) {
                                    loginOnline(res);
                                } else {
...
function parseTokenJSON(response) {
    ...
    console.log("Actualizar es : "+ accion);
