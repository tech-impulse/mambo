...
function _createTableRelCatalog(backUp){
	 ...
	 var sql = 
        ...
        "  ordinalType VARCHAR(10), " +
        "  isPrimary  TINYINT, " +


                 "  logisticsChainName VARCHAR(50), " +
                 "  mainItemEan VARCHAR(50), " +
...
function getQueryInsertCatalog(data) {
    ...
    else { data.itemName=data.itemName.replace(/""/g,"´"); data.itemName=data.itemName.replace(/''/g,"´"); }

    if (data.logisticsChainName===undefined) { data.logisticsChainName=""; }
    else {  data.logisticsChainName=data.logisticsChainName.replace(/''/g,"´");  data.logisticsChainName=data.logisticsChainName.replace(/""/g,"´"); }
	...
	var q='INSERT OR IGNORE INTO catalog ( idItem , idPurchaseCenter , idVendor , idLogisticsChains ,  vendorReference , vendorStatus  , primaryVendorId , isElectronicVendor , isByEmailVendor , theoreticalStock ,  isAuthorizedVendor , ' +
 				...
 				' currency , warehouseId , allowsPerUnitRequest ,   purchaseCenterItemStatus , itemStatus, documentInterfaceType , itemUnitName, itemName, logisticsChainName, mainItemEan, ordinalType, isPrimary ) '+
				...
				' "'+ data.currency+'", "'+ data.warehouseId+'", "'+ data.allowsPerUnitRequest+'", "'+ data.purchaseCenterItemStatus+'", "'+ data.itemStatus+'", "'+ data.documentInterfaceType+'", "'+ data.itemUnitName+'", "'+data.itemName+'", "'+data.logisticsChainName+'", "'+ data.mainItemEan+'", "'+data.ordinalType+'", "'+ data.isPrimary+'" )';
